...
def parse_raw_vocab_line(s: str) -> pd.DataFrame:
    ...
    name = re.sub(pattern=r"^([a-z]+) .*", repl="\\1", string=s).strip()
    pos = re.sub(r"^[a-z]+ \(([a-z]+)\):.*", "\\1", s).strip()
    # Definitions can be dirty (contain punctuation before)
    word_def = re.sub(r"^[a-z]+ \(([a-z]+)\):(.*)", "\\2", s).strip()
    word_def = re.sub(r"^(.*) \..*", "\\1", word_def.lstrip(".,")).strip()

    return pd.DataFrame({"word": name, "pos": pos, "definition": word_def}, index=[0])
...
def get_raw_vocab(out_file: str = None) -> pd.DataFrame:
    ...
    for section, data_file in data_files.items():
        ...
        with open(data_file, "r") as f:
            all_vocab = pd.concat([parse_raw_vocab_line(line) for line in f])
