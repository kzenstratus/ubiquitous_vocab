...
def _get_sents_containing_word(all_text : str,
    ...
    sim_scores = get_cos_sim(text_a = all_orig_text,
                            text_b = all_mod_text,
                            st = st)
    
    log.info(f"word: {len(all_words)}, syn : {len(all_syn)}, orig_text : {len(all_orig_text)}, mod_text : {len(all_mod_text)}, sim_score : {len(sim_scores)}")
    # TODO: either store replaced context as a list of replacedcontext obj
    # or just as a dataframe. 
    article_df = pd.DataFrame(dict(word = all_words,
                                     syn = all_syn,
