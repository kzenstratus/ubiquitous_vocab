...
def _get_sents_containing_word(all_text : str,
    ...
    sent_len = len(sentences) - 1
    
    all_replaced_context = []
    # For each synonym, go through each sentence to see if that     
    for syn, word in word_map.items():
        all_orig_text = []
        all_mod_text = []
        for sent_id, sent in enumerate(sentences):
            
            # Only look for full words (ie. \b)
            regex_syn = f"\\b({syn})\\b"
            if re.search(regex_syn, sent) is not None:
                ...
                last_sent = min(sent_id + more_sent, sent_len)
...
def get_replace_example(news_df : pd.DataFrame,
    ...
    orig_highlights = _get_sents_containing_word(tmp["article"].array[0],
                                word_map = syn_to_word_regex)
    
    tmp["orig_highlights"] = orig_highlights

    # \\b is necessary to match full words.    
    tmp["new_article"] = tmp.article.replace({f"\\b{k}\\b": v for k,v in syn_to_word_regex},
                                             regex = True)
